...
•	Word-count doesn’t apply well to languages like Japanese or Hindi, where white place is typically used between whole sentences instead of words.
...
•	Run the programme with ‘python3 main.py <file_path>’.

•	Current books can be found in ‘books’ while test documents for languages can be found in ‘testbooks’.
e.g. 
‘python3 main.py books/prideandprejudice.txt’
‘python3 main.py testbooks/examplejapanese.txt’

Key lessons/insights: 
•	Learnt that codes and functions should be piped/imported in a single direction, not imported reciprocally, creating circular dependencies.
